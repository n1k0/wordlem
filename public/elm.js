...
var $author$project$Main$randomWord = A2(
	...
	A2(
		...
		0,
		$elm$core$List$length($author$project$Main$words) - 1));
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{attempts: _List_Nil, currentTry: '', error: $elm$core$Maybe$Nothing, gameState: $author$project$Main$Ongoing, word: $elm$core$Maybe$Nothing},
...
var $author$project$Main$try = F2(
	function (word, input) {
		...
		return A2(
			...
			$elm$core$String$toLower(input),
			$author$project$Main$words)) ? $elm$core$Result$Err('Sorry, ' + (input + ' must be a word known from our dictionnary')) : $elm$core$Result$Ok(
			A3(
				$elm$core$List$map2,
				F2(
					function (a, b) {
...
var $author$project$Main$update = F2(
	function (msg, model) {
		...
		var currentTry = model.currentTry;
		switch (msg.$) {
			case 'NewGame':
				...
				if (msg.a.$ === 'Just') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								word: $elm$core$Maybe$Just(string)
							}),
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								gameState: $author$project$Main$Errored('There was an issue while picking a word')
							}),
						$elm$core$Platform$Cmd$none);
				}
			case 'UpdateTry':
			...
			default:
				if (msg.a.$ === 'Just') {
					...
					var _v2 = A2($author$project$Main$try, wordToFind, currentTry);
					if (_v2.$ === 'Ok') {
						var attempt = _v2.a;
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									attempts: A2($elm$core$List$cons, attempt, attempts),
									currentTry: '',
...
var $author$project$Main$gameState = function (model) {
	return $author$project$Main$hasWon(model.attempts) ? $author$project$Main$Won : ((_Utils_cmp(
		$elm$core$List$length(model.attempts),
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Guess a 5 letters English word in '),
						A2(
						...
						return A2(
							...
							_List_fromArray(
								[
									...
									_List_fromArray(
										[
											$elm$html$Html$text('Enter a 5 letters English word')
										]))
								]));
				}
			}()
